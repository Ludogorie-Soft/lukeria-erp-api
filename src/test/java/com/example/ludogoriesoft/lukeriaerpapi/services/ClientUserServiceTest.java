...
class ClientUserServiceTest {
  ...
  void setUp() {
    ...
    clientUserService =
        new ClientUserService(clientUserRepository, userRepository, clientRepository, modelMapper);
  }
  ...
  void testGetClientUserById_Existing() throws ChangeSetPersister.NotFoundException {
    ...
    when(clientUserRepository.findByIdAndDeletedFalse(clientUserId))
  ...
  void testGetClientUserById_NonExisting() {
    ...
    assertThrows(
        ...
        () -> clientUserService.getClientUserById(clientUserId));
  }
  ...
  void testCreateClientUser_Success() {
    ...
    when(clientRepository.findByIdAndDeletedFalse(clientUserDTO.getClientId()))
        .thenReturn(Optional.of(client));
    when(userRepository.findByIdAndDeletedFalse(clientUserDTO.getUserId()))
  ...
  void testCreateClientUser_NotFoundException() {
    ...
    assertThrows(
        ChangeSetPersister.NotFoundException.class,
        () -> clientUserService.getClientUserById(null));
  }
  ...
  void testCreateClientUser_RoleAdmin() {
    ...
    // Act
    assertThrows(
        ValidationException.class, () -> clientUserService.createClientUser(clientUserDTO));
  }
  ...
  void testUpdateClientUser_Success() throws ChangeSetPersister.NotFoundException {
    ...
    when(clientUserRepository.findByIdAndDeletedFalse(existingClientUser.getId()))
  ...
  void testUpdateClientUser_NonExisting() {
    ...
    assertThrows(
        ChangeSetPersister.NotFoundException.class,
        () -> clientUserService.updateClientUser(clientUserId, clientUserDTO));
  }

  @Test
   void deleteClientUser_ShouldDeleteClientUser_WhenExists() {
    Long userId = 1L;
    Long clientId = 1L;
    ClientUser clientUser = new ClientUser();
    clientUser.setId(1L);
    when(clientUserRepository.findByUserIdAndClientId(userId, clientId)).thenReturn(clientUser);

    clientUserService.deleteClientUser(userId, clientId);

    verify(clientUserRepository, times(1)).delete(clientUser);
  }

  @Test
   void deleteClientUser_ShouldNotDelete_WhenNotExists() {
    Long userId = 1L;
    Long clientId = 1L;
    ClientUser clientUser = new ClientUser();
    clientUser.setId(1L);    when(clientUserRepository.findByUserIdAndClientId(userId, clientId)).thenReturn(null);

    clientUserService.deleteClientUser(userId, clientId);

    verify(clientUserRepository, never()).delete(any());
  }
}
